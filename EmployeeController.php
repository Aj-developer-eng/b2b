...
///needs to be check the blow code before activating it
    ...
    { 
        ...
        $find_lead_email0 = DB::table('task_details')->where('id','=', $request->user_id)->get();
        //dd($find_lead_email0);
        $find_lead_email1 = $find_lead_email0[0]->employee_id;
        $find_mandoob_detail = DB::table('employees')->where('id','=', $find_lead_email1)->get();
        $accept = [
        ...
        \Mail::to($request->email)->send(new acceptmailtolead($accept));

        $find_agent_detail = DB::table('tasks')->where('employee_id','=', $find_lead_email1)->select('agent_id')->get();
        $find_agent_id = $find_agent_detail[0]->agent_id;
        $find_agent_email = DB::table('agents')->where('id','=', $find_agent_id)->select('email')->get();
        //dd($find_lead_email0);
        $lead_name = $find_lead_email0[0]->lead_p_name;
        $lead_email = $find_lead_email0[0]->email;
        $lead_contact = $find_lead_email0[0]->contact;
        $lead_note = $find_lead_email0[0]->note;
        $toagent = [
        ...
        \Mail::to($find_agent_email[0]->email)->send(new acceptmailtoagent($toagent));   
    }
}
